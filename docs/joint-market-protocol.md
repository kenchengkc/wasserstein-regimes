...
The development command fits/selects models and seals arrays, configuration, diagnostics and provenance. The explicit assessment command requires that artifact, validates its configuration and hashes, loads models, and only predicts. Default execution stops after development. Dataset changes fail; published data remain aggregate only. Save per-window assignments locally for audits.
